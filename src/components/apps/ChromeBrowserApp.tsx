...
const ChromeBrowserApp: React.FC<{ windowId: string; appKey: string }> = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-background text-foreground overflow-hidden">
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8" disabled={activeTab === 'google'}>
                <ArrowLeft size={18} />
              </Button>
            </TooltipTrigger>
            <TooltipContent>
          ...
          <div className="relative flex-grow mx-2">
            ...
            <Input
              ...
              className="h-8 w-full pl-9 pr-2 rounded-full bg-background/80 dark:bg-zinc-700/50 border-border focus-visible:ring-primary transition-all duration-200"
      ...
      {isLoading ? (
        <div className="flex-grow overflow-y-auto bg-background flex items-center justify-center p-8">
        ...
        <div className="flex-grow overflow-y-auto bg-background flex flex-col items-center justify-center p-8">
          <div className="text-center w-full max-w-2xl">
            <svg 
              className="mx-auto h-20 w-auto text-muted-foreground/50 mb-6 transition-all duration-300 hover:scale-105" 
              viewBox="0 0 91 30" 
              fill="none" 
            ...
            <div className="relative w-full mx-auto my-6">
              ...
              <Input
                type="search"
                placeholder="Search Google or type a URL"
                className="w-full h-12 pl-12 pr-4 rounded-full shadow-md bg-muted/50 dark:bg-zinc-700/60 border-transparent focus-visible:ring-2 focus-visible:ring-primary focus-visible:border-primary transition-all duration-200 hover:shadow-lg"
                readOnly
              />
            </div>
                ...
                {mockShortcuts.map((shortcut) => (
                  <Tooltip key={shortcut.name}>
                    <TooltipTrigger asChild>
                      <Button
                        variant="ghost"
                        className="flex flex-col items-center justify-center h-24 w-full p-2 space-y-1 hover:bg-muted/70 dark:hover:bg-zinc-700/80 rounded-lg transition-all duration-200 hover:scale-105"
                        onClick={() => handleShortcutClick(shortcut.url)}
                      >
                        <div className={`p-3 rounded-full ${shortcut.color} mb-1 transition-transform duration-200 group-hover:scale-110`}>
                          {shortcut.icon}
                        </div>
                        <span className="text-xs text-foreground truncate w-full px-1">{shortcut.name}</span>
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>{shortcut.url}</p>
        ...
        <div className="flex-grow overflow-y-auto bg-background flex items-center justify-center p-8">
          <Card className="w-full max-w-4xl">
            <CardContent className="p-6 text-center">
              <div className="text-2xl font-semibold mb-2">Browser Page</div>
              <p className="text-muted-foreground mb-4">Currently viewing: {activeTab}</p>
              <Button 
                variant="outline" 
